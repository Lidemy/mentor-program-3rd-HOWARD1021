...
function render() {
  ...
  for (let i = 0; i < list.length; i += 1) {
    $('ul').append(
      `<li class="list-group-item d-flex justify-content-between align-items-center  ${list[i].isDone ? 'checked' : ''}" data-id="${list[i].dataId}">
...
function addTodo(todo) {
  const obj = {
    dataId: Math.random().toString(36).substr(2, 5),
    content: todo,
    isDone: false,
  };
  list.push(obj);
...
// ${list[i].isDone ? 'checked' : ''}" data-id="${i}">
//     <div class='inputcheck'>
//     <input type="checkbox" name="" value=""
// class = 'inputcheckout' ${list[i].isDone ? 'checked' : ''}>
//     <span class ='item-name ' >${list[i].content}</span>
//     </div>
//     <button type="button" class="close" aria-label="Close">x
// </button>
