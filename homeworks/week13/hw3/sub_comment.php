...
<?php
/*當輸入子留言的時候 將 $parent_id 宣告成　$row['id']*/
$parent_id = $row['id'];
...
if ($result_sub->num_rows > 0) {
    ...
    if ($row_sub['username'] === $row['username']) {
         ...
         echo "<div class='sub_commentonce'>"; 
		...
		echo "<button class='btn message__delete btn-danger' data-id='" . $row_sub['id'] ."'>delete</button>";

		echo "<button class='btn  btn-light'><a href='./update_post.php?id= " . $row['id'] . "'> Edit </a></button>";
// div 位置如果放這的話 會造成使用者,會看到像是撲克牌結束的畫面 //
		}
		echo "</div>";
    }
}
?>
	<form method = 'POST' action='./handle_add.php'　class='sub_comment_add' name="comments">
	...
	<?php
   if ($user) {
          echo "<input type='submit' value='留言' class='btn btn-primary'  >";
      } else {
